...
function stageMarkup(stage) {
  ...
  if (stage.id === "mail") return `<section class="experience-stage stage-mail">${bg}<div class="stage-overlay">${echoMarkup()}<button class="hotspot envelope-surface" data-action="envelope" aria-label="封筒をなでる"></button><button class="lottery-flap guided-control" data-action="flap" aria-label="封筒のふたを上へ開く"><b>↑</b><small>ふたを上へ<br><em>タップでもOK</em></small></button><div class="lottery-result-sheet" data-action="paper"><img src="${asset(state.result ? stage.images.win : stage.images.lose)}" alt="抽選結果"><span>↑ 紙を上へ引く<br><em>タップでも開く</em></span></div></div></section>`;
...
function instruct(text){ el.stageWhisper.innerHTML=`<strong>次の操作</strong><span>${text}</span>`; }
...
function bindGacha(root) {
  ...
  let doorBase=0;
  const openDoor=()=>{state.log.doorOpenMs=performance.now()-state.resolvedAt;state.phase="card";root.classList.add("door-open","card-ready");sound("door");vibrate(10);instruct("中央の裏向きカードをタップする");};
